...
async function performAIScreening(
  ...
  try {
    ...
    return {
      ...
      strengths: result.skills_found?.length ? 
        [`Found ${result.skills_found.length} required skills`] :
        [],
      weaknesses: result.skills_missing?.length ?
      ...
      extracted_info: {
        ...
        years_experience: Number(result.years_of_experience) || 0,
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!remainingCandidates || remainingCandidates.length === 0) {
      ...
      const successCount = (allCandidates || []).filter((c: CandidateStatus) => c.status === "completed").length;
      const failedCount = (allCandidates || []).filter((c: CandidateStatus) => c.status === "failed").length;
